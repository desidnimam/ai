...
import { FlexGrid, FlexGridTWO, TabGrid } from "./flex-grid";
...
const components: MDXComponents = {
  ...
  code: ({ className, ...props }: React.HTMLAttributes<HTMLElement>) => (
    <code
      className={cn(
        "relative rounded-md border border-slate-200 px-[0.4rem] py-[0.2rem] font-mono text-sm dark:border-slate-800",
        className,
      )}
      {...props}
    />
    ...
    return (
      <>
        <ImageZoom>
          <Image
            src={src}
            className="not-prose bg-aired flex h-full w-full cursor-zoom-in rounded-3xl border border-slate-200 object-cover object-center dark:border-slate-800"
            alt={alt}
            {...rest}
          />
        </ImageZoom>
