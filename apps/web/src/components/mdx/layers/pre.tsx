...
const Pre = (props: PreProps) => {
  ...
  return (
    <figure className="not-prose group relative my-6 overflow-hidden rounded-2xl border border-slate-400 text-sm dark:border-slate-600">
      {title ? (
...
const CopyButton = (props: CopyButtonProps) => {
  ...
  return (
    <Button
      className={cn(
        "bg-aired size-8 p-0 opacity-0 transition-opacity group-hover:opacity-100",
        className,
      )}
      variant="outline"
      onClick={() => copy({ text })}
